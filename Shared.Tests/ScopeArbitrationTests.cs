    ...
    public class ScopeArbitrationTests
    {
        ...
        private static FederationPeer Peer(long pbId, ulong signature, params long[] grids)
        {
            return new FederationPeer
            {
                PbId = pbId,
                Signature = signature,
                ConstructGrids = new HashSet<long>(grids)
            };
        }
        ...
        {
            var peers = new List<FederationPeer> { Peer(200, 55UL) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, new List<ConnectorEdge>(), peers);
        ...
        {
            var peers = new List<FederationPeer> { Peer(MyPb, 55UL) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, new List<ConnectorEdge>(), peers);
        ...
        {
            var peers = new List<FederationPeer> { Peer(200, 99UL) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, new List<ConnectorEdge>(), peers);
            result.DuplicateScope.Should().BeFalse();
        }
        ...
        {
            var conn = new List<ConnectorEdge> { Link(0, 0) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 1UL, conn, new List<FederationPeer>());
        ...
        {
            var conn = new List<ConnectorEdge> { Link(0, 0, otherTag: false) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 1UL, conn, new List<FederationPeer>());
        ...
        {
            var conn = new List<ConnectorEdge> { Link(0, 0, connected: false) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 1UL, conn, new List<FederationPeer>());
            result.ApprovedFederateGrids.Should().BeEmpty();
        }
        ...
        {
            var conn = new List<ConnectorEdge> { Link(localPriority: 1, otherPriority: 0) };
            var peers = new List<FederationPeer> { Peer(200, 99UL, 9) };
        ...
        {
            var conn = new List<ConnectorEdge> { Link(localPriority: 0, otherPriority: 0) };
            var peers = new List<FederationPeer> { Peer(200, 99UL, 9) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, conn, peers);
            result.StandDown.Should().BeFalse();
            result.ApprovedFederateGrids.Should().BeEmpty();
        ...
        {
            var conn = new List<ConnectorEdge> { Link(localPriority: 0, otherPriority: 2) };
            var peers = new List<FederationPeer> { Peer(200, 99UL, 9) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, conn, peers);
            result.StandDown.Should().BeFalse();
            result.ApprovedFederateGrids.Should().BeEquivalentTo(new long[] { 9 });
        ...
        public void Any_outranking_neighbor_clears_all_approvals()
        {
            ...
            var peers = new List<FederationPeer> { Peer(200, 99UL, 8), Peer(300, 88UL, 9) };
            ArbitrationResult result = ScopeArbitration.Decide(Root, MyPb, 55UL, conn, peers);
            result.StandDown.Should().BeTrue();
            result.ApprovedFederateGrids.Should().BeEmpty();
        }
