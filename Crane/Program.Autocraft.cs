    ...
    public partial class Program : MyGridProgram
    {
        ...
        private IEnumerable<YieldReason> ParseCCraftCustomData(string customData)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (!QuotaParsing.IsValidQuotaKey(key))
                {
                    continue;
                }
                ...
                if (!QuotaParsing.TryParseQuotaKeyShape(key, out typeIdWithPrefix, out subtypeId))
                {
                    continue;
                }
        ...
        private IEnumerable<YieldReason> SyncCCraftTemplate(IMyTerminalBlock host)
        {
            ...
            foreach (string k in _catalog.KnownItems.Keys)
            {
                _ccraftMergedKeys.Add(k);
            }

            foreach (string k in _autocraftActiveQuotaLines.Keys)
            {
                _ccraftMergedKeys.Add(k);
            }

            _ccraftSortedKeys.Clear();
            foreach (string k in _ccraftMergedKeys)
            {
                _ccraftSortedKeys.Add(k);
            }
        ...
        private IEnumerator<YieldReason> StepQuotaEngine()
        {
            _quotaStatus.Clear();
            _quotaQueued.Clear();
        ...
        private IEnumerator<YieldReason> StepAssemblerPool()
        {
            ...
            if (!diverged)
            {
                ...
                {
                    if (!_capabilityCacheShadow.Contains(id))
                    { diverged = true; break; }
                }
            }
            if (diverged)
            {
                ...
                foreach (long id in currentSet)
                {
                    _capabilityCacheShadow.Add(id);
                }

                if (!currentSet.Contains(_reservedAsmEntityId))
                {
        ...
        private IEnumerator<YieldReason> StepDispatchAndReconcile()
        {
            ...
            if (reservedActive)
            {
                _reservedAsmEntityId = reservedId;
            }
            else
            {
                _reservedAsmEntityId = 0;
        ...
        private long SelectReservedAssembler(List<AutocraftTarget> disassembleWork, bool reservedActive)
        {
            if (!reservedActive || disassembleWork.Count == 0 || _assemblers.Count == 0)
            {
                return 0;
            }
            ...
            if (_reservedAsmEntityId != 0)
            {
                for (int i = 0; i < _assemblers.Count; i++)
                {
                    if (_assemblers[i].EntityId == _reservedAsmEntityId)
                    {
                        return _reservedAsmEntityId;
                    }
                }
            }
            long best = 0;
            ...
            for (int i = 0; i < _assemblers.Count; i++)
            {
                ...
                {
                    if (IsAssemblerCapable(_assemblers[i], disassembleWork[k]))
                    {
                        score++;
                    }
                }
            ...
            out List<IMyAssembler> assembleSlice, out List<IMyAssembler> disassembleSlice)
        {
            ...
            for (int i = 0; i < _assemblers.Count; i++)
            {
                ...
                if (reservedActive && a.EntityId == reservedId)
                {
                    continue;
                }

                if (assembleSlice.Count < assembleCount)
                {
                    assembleSlice.Add(a);
                }
                else if (disassembleSlice.Count < (disassembleCount))
                {
                    disassembleSlice.Add(a);
                }
            }
        }
        ...
        private void ReconcileAssembler(IMyAssembler asm, AutocraftTarget t, long share, MyAssemblerMode mode)
        {
            if (asm == null || share <= 0)
            {
                return;
        ...
        private bool CanFlipMode(IMyAssembler asm, MyAssemblerMode desired)
        {
            if (asm.Mode == desired)
            {
                return true;
        ...
        private bool IsAssemblerCapable(IMyAssembler asm, AutocraftTarget t)
        {
            if (asm == null)
            {
                return false;
        ...
        private long SumQueuedAmount(AutocraftTarget t)
        {
            long total = 0;
            for (int i = 0; i < _assemblers.Count; i++)
            {
        ...
        private readonly List<MyProductionItem> _queueBuffer = new List<MyProductionItem>();

        /// <summary>Sums the queued amount of <paramref name="t"/>'s blueprint on a single assembler.</summary>
        private long SumQueuedAmountOnAssembler(IMyAssembler asm, AutocraftTarget t)
        {
            if (asm == null)
            {
                return 0;
        ...
        private static bool BlueprintSubtypeMatchesItem(string blueprintSubtype, string itemSubtype)
        {
            if (blueprintSubtype == itemSubtype)
            {
                return true;
            }
