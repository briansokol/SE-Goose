...
namespace IngameScript {
    public partial class Program : MyGridProgram {
        ...
        readonly Dictionary<string, string> _blueprintMap = new Dictionary<string, string>(StringComparer.Ordinal);

        /// <summary>Catalog keys flagged as needing operator-driven manual seeding.</summary>
        readonly HashSet<string> _needsLearn = new HashSet<string>(StringComparer.Ordinal);
        ...
        MyDefinitionId? ResolveBlueprintForTarget(IMyAssembler asm, AutocraftTarget t) {
            string cached;
        ...
        static MyDefinitionId? TryMakeBlueprintId(string subtype) {
            if (string.IsNullOrEmpty(subtype)) return null;
            MyDefinitionId id;
            if (MyDefinitionId.TryParse("MyObjectBuilder_BlueprintDefinition/" + subtype, out id)) return id;
            return null;
        }
        ...
        void PassiveLearnFromQueues() {
            if (_needsLearn.Count == 0) return;
            for (int a = 0; a < _assemblers.Count; a++) {
                IMyAssembler asm = _assemblers[a];
                if (asm == null) continue;
                _queueBuffer.Clear();
                try { asm.GetQueue(_queueBuffer); } catch { continue; }
                for (int q = 0; q < _queueBuffer.Count; q++) {
                    ...
                    foreach (string key in _needsLearn) {
                        if (_blueprintMap.ContainsKey(key)) continue;
                        string itemSub;
                        if (!TryGetItemSubtypeForKey(key, out itemSub)) continue;
                        if (bpSub == itemSub) {
            ...
            foreach (string key in _needsLearn) {
                if (_blueprintMap.ContainsKey(key)) {
                    if (learned == null) learned = new List<string>();
                    learned.Add(key);
                }
            }
            if (learned != null) {
        ...
        static bool TryGetItemSubtypeForKey(string key, out string subtype) {
            subtype = null;
        ...
        IEnumerator<YieldReason> StepRenderStatus() {
        ...
        void RenderCCraftStatus() {
            ...
            for (int i = 0; i < _autocraftTargets.Count; i++) {
                ...
                if (queued > 0) sb.Append(" (").Append(queued).Append(" queued, ").Append(asmCount).Append(" asm)");
                sb.Append('\n');
            }
            string text = sb.ToString();
            for (int i = 0; i < _ccraftLcds.Count; i++) {
                if (_ccraftLcds[i] != null) _ccraftLcds[i].WriteText(text, false);
            }
        }

        /// <summary>Writes the warning log to every <c>[CError]</c> LCD surface.</summary>
        void RenderCErrorStatus() {
            if (_cerrorLcds.Count == 0) return;
            StringBuilder sb = new StringBuilder();
            sb.Append("=== Crane Errors ===\n");
            foreach (var kv in _logger.Warnings) {
                sb.Append(kv.Key);
            ...
            for (int i = 0; i < _cerrorLcds.Count; i++) {
                if (_cerrorLcds[i] != null) _cerrorLcds[i].WriteText(text, false);
            }
        }
        ...
        void PersistBlueprintsSection() {
            if (_ccraftConfigHost == null) return;
        ...
        void LoadBlueprintsSection(string customData) {
        ...
        static string StripSectionVerbatim(string customData, string sectionHeader) {
            if (string.IsNullOrEmpty(customData) || string.IsNullOrEmpty(sectionHeader)) return customData ?? string.Empty;
            string[] lines = customData.Split('\n');
            int start = -1;
            for (int i = 0; i < lines.Length; i++) {
                string trimmed = lines[i].TrimEnd('\r').Trim();
            ...
            for (int i = end; i < lines.Length; i++) sb.Append(lines[i]).Append(i < lines.Length - 1 ? "\n" : "");
            return sb.ToString();
        }
        ...
        static string ShortenKey(string key) {
            ...
            int slash = key.IndexOf('/');
            if (slash < 0 || slash >= key.Length - 1) return key;
        ...
        string FindAssemblerName(long entityId) {
            for (int i = 0; i < _assemblers.Count; i++) {
                if (_assemblers[i] != null && _assemblers[i].EntityId == entityId) return _assemblers[i].CustomName;
            }
