...
namespace IngameScript
{
    public partial class Program : MyGridProgram
    {
        ...
        /// </summary>
        private IEnumerator<YieldReason> StepBalanceSameRoleContainers()
        {
            ...
            foreach (KeyValuePair<ItemCategory, ContainerList> kv in _containersByCategory)
            {
                ...
                for (int r = 0; r < routes.Count; r++)
                {
                    ...
                    if (e.ConsumerKind != ConsumerKind.None)
                    {
                        continue;
                    }

                    if (!ValidateBlock(e.Block))
                    {
                        continue;
                    }
                ...
                if (!WillSameRoleBalancingRun(true, _roleBalanceCandidates.Count))
                {
                    continue;
                }
        ...
        private IEnumerator<YieldReason> BalanceCategoryByTier(ItemCategory category, ContainerList candidates)
        {
            ...
            while (i < n)
            {
                ...
                while (i < n && candidates[i].Priority == tierPriority)
                {
                    i++;
                }
                ...
                for (int t = tierStart; t < tierEnd; t++)
                {
                    _roleBalanceTier.Add(candidates[t]);
                }

                _roleBalanceLowerTiers.Clear();
                for (int t = tierEnd; t < n; t++)
                {
                    _roleBalanceLowerTiers.Add(candidates[t]);
                }
        ...
        private IEnumerator<YieldReason> PullUpIntoTier(ItemCategory category, ContainerList upperTier, ContainerList lowerTiers)
        {
            ...
            for (int l = 0; l < lowerTiers.Count; l++)
            {
                ...
                for (int k = 0; k < _itemBuffer.Count; k++)
                {
                    ...
                    if (!_roleBalanceHoldings.ContainsKey(t))
                    {
                        _roleBalanceHoldings[t] = null;
                    }
                }
            }
        ...
        private long ProbeAdditionalCapacity(IMyInventory inv, MyItemType type)
        {
            ...
            if (inv.CanItemsBeAdded((MyFixedPoint)(double)RoleBalanceProbeMax, type))
            {
